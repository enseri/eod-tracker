...
async function claimStreakMilestoneNotification(companyId, userId, notifyKey) {
  ...
  await mutateStore(
    ...
    (latest) => {
      ...
      const keys = Array.isArray(user?.streakMilestoneNotifiedKeys)
        ? [...user.streakMilestoneNotifiedKeys]
        : [];
      if (keys.includes(notifyKey)) {
        claimed = false;
        return {};
      }
      claimed = true;
      return {
        userPatches: {
          [userId]: { streakMilestoneNotifiedKeys: [...keys, notifyKey] },
        },
      };
    },
    (check) => !!(check.users?.[userId]?.streakMilestoneNotifiedKeys || []).includes(notifyKey),
  );
  return claimed;
}
...
module.exports = {
  ...
  isUserPro,
  storageMode,
  claimStreakMilestoneNotification,
  hasBlobConfig,
  emptyStore,
  defaultUser,
};
