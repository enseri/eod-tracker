...
/**
 * Streak emoji grows with the number (manager spec):
 *   seedling → fire (30) → volcano (90) → comet (180) → stone statue (365).
 */
...
/** Dedup key for milestone chat + in-app toast (e.g. `7@2026-06-25`). */
function milestoneNotifyKey(milestone, dateStr) {
  if (!milestone?.days || !dateStr) return null;
  return `${milestone.days}@${dateStr}`;
}
